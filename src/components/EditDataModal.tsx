...
interface EditDataModalProps {
    isOpen: boolean;
    onClose: () => void;
    handleEditUpdate: (data: Data) => void;
    existingData: Data | null;
}

const EditDataModal: React.FC<EditDataModalProps> = ({ isOpen, onClose, handleEditUpdate, existingData }) => {
                        ...
                        >
                            <DialogPanel className="w-full max-w-md rounded-xl text-black p-6 backdrop-blur-2xl bg-white">
                                ...
                                <div className='w-full flex flex-col justify-center'>
                                    ...
                                    <div>
                                        <label htmlFor="name" className=" block mb-2 text-sm font-medium text-gray-900">Edit Name</label>
                                        <input
                                            id="name"
                                            type="text"
                                            value={data.name}
                                    ...
                                    <div className="flex flex-col w-full">
                                        <label className="w-full block mt-2 mb-2 text-sm font-medium text-gray-900">Edit Date Of Birth</label>
                                        <input
                                            type="date"
                                            value={data.dob}
                                    ...
                                    <Button
                                        className="mt-4 inline-flex items-center justify-center gap-2 rounded-md bg-gray-700 py-1.5 px-3 text-sm/6 font-semibold shadow-inner shadow-white/10 focus:outline-none data-[hover]:bg-gray-600 text-white data-[open]:bg-gray-700 data-[focus]:outline-1 data-[focus]:outline-white disabled:cursor-not-allowed"
                                        onClick={handleSubmit}
                                        disabled={!data.name || !data.dob || loading}
                                    >
                                        {loading ? (
                                            <div role="status">
